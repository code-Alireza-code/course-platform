...
async function CourseGrid() {
  ...
  return courses.map((course) => (
    <Card key={course.id} className="overflow-hidden flex flex-col">
      ...
      <CardContent className="line-clamp-3" title={course.descripton}>
        {course.descripton}
      </CardContent>
...
async function getUserCourses(userId: string) {
  ...
  const courses = await db
    .select({
      ...
      descripton: CourseTable.description,
    ...
    .leftJoin(
      LessonTable,
      and(eq(LessonTable.sectionId, CourseTable.id), wherePublicCourseSections)
    )
    .leftJoin(
      UserLessonCompleteTable,
      and(
