...
export async function deleteSection(id: string) {
  const [deletedeSection] = await db
    .delete(CourseSectionTable)
    .where(eq(CourseSectionTable.id, id))
    .returning();

  if (deletedeSection == null)
    throw new Error("failed to delete your section !");

  revalidateCourseSectionCache({
    id: deletedeSection.id,
    courseId: deletedeSection.courseId,
  });

  return deletedeSection;
}
